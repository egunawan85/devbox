...
# (DigitalOcean/Azure) lives under providers/, OS-specific code under os/ — both are
# sourced by load_conf once the profile's PROVIDER/OS are known.
#
...
load_conf() {
  ...
  # Source the provider module for this profile (the OS module joins it once os/ is
  # populated in a later slice). Provider name validated above (lowercase only).
  if [ -z "${_PROVIDER_LOADED:-}" ]; then
    local provmod="$SCRIPT_DIR/providers/$PROVIDER.sh"
    [ -f "$provmod" ] || die "no provider module for PROVIDER='$PROVIDER' ($provmod)"
    # shellcheck disable=SC1090
    . "$provmod"; _PROVIDER_LOADED=1
  fi
}
...
wait_box_ready() {
  ...
  while :; do
    box_ready "$ip" && { log "box is ready"; return 0; }
...
cmd_up() {
  ...
  if [ -n "$(prov_exists)" ]; then
    ...
    # An existing box may still be mid first-boot; gate on readiness before configure.
...
cmd_configure() {
  ...
  [ "$OS" = linux ] || die "configure: OS '$OS' is not wired yet — the os/$OS.sh module lands in a later slice"

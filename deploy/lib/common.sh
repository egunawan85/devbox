...
# (DigitalOcean/Azure) lives under providers/, OS-specific code under os/.
#
...
resolve_conf() {
  ...
  TEMPLATE=$SCRIPT_DIR/cloud-init.yaml           # linux first-boot template (moves to os/linux.sh in a later slice)
}
...
load_conf() {
  ...
  # If present, `configure`/`up` installs the login-time materializer (see install_session_secrets).
...
# A box is READY when cloud-init is done AND the devbox-ready marker exists. We probe
# readiness over SSH (non-blocking status check, so we can interleave a liveness probe).
box_ready() {
...
# Create the droplet and bring it to ready, auto-recreating on a bad VM. Sets the global
# PROVISIONED_IP on success; dies on failure. Manages the temporary ICMP liveness probe
# (added once, removed at script EXIT so it's cleaned even on error).
provision_droplet() {
...
cmd_up() {
  ...
  if [ -n "$(droplet_id)" ]; then
    ...
    # An existing box may still be mid cloud-init; gate on devbox-ready before configure.
    wait_for_box "$ip" off || die "existing droplet '$DROPLET_NAME' ($ip) is not becoming ready — it may be wedged; '$(basename "$0") down' then '$(basename "$0") up' to recreate"
  else
    provision_droplet           # sets PROVISIONED_IP; auto-recreates on a bad VM; dies on failure
    ip="$PROVISIONED_IP"
  fi
...
EOF
  install_session_secrets "$host"   # opt-in: only if a local secrets.map exists
...
vault_bringup() {
  ...
  autoseal_arm "$host"   # (re)arm the auto-seal timer if AUTOSEAL_TTL is set
}
